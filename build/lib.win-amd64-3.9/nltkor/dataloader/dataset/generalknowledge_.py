...
class GeneralKnowledge(DataDownload) :
    ...
    def download_(self,root_dir = None):
        if not self.check_canard_():        
            ...
            base_url = "https://air.changwon.ac.kr/~airdemo/storage/data/지식베이스/일반상식.zip"
            print("Downloading...")
            ...
            with requests.get(base_url, stream=True) as r:
                        ...
                        with open(file_path, 'wb') as f:
                                ...
                                #if chunk:
                                        f.write(chunk)
            self.extract_and_handle_zip(file_path)
        else :
            print("already downloaded data")   
                    
if __name__ == '__main__':
    _generalknowledge_ = GeneralKnowledge() 
    _generalknowledge_.download_() 
