...
def convert_morph_lexicon(filename, pkl_filename):
	morph_dict = {} 
...
def load_morph_lexicon(filename):
	pkl_filename = filename.replace('.txt', '.pickle')
	convert_morph_lexicon(filename, pkl_filename) 

	with open(pkl_filename, 'rb') as f:
		morph_dict = _pickle.load(f)
	return morph_dict 


def convert_co_lexicon(filename, pkl_filename):
	co_morph_dict = {} 
	...
	with open(filename, 'rt', encoding=detected) as f:
		for line in f:
			...
			if k in co_morph_dict:
				print(f"load co morph lexicon : key {k} conflict!".format(k)) 
			co_morph_dict[k] = v

	with open(pkl_filename, 'wb') as f:
		_pickle.dump(co_morph_dict, f, 2)

def load_co_lexicon(filename):
	pkl_filename = filename.replace('.txt', '.pickle')
	convert_co_lexicon(filename, pkl_filename) 

	with open(pkl_filename, 'rb') as f:
		co_morph_dict = _pickle.load(f)
	return co_morph_dict 


def convert_prob_dict(filename, pkl_filename):
	prob_dict = {} 
	...
	with open(pkl_filename, 'wb') as f:
		_pickle.dump(prob_dict, f, 2)

def load_prob_dict(filename):
	pkl_filename = filename.replace('.txt', '.pickle')
	convert_prob_dict(filename, pkl_filename) 

	with open(pkl_filename, 'rb') as f:
		prob_dict = _pickle.load(f)
	return prob_dict 
